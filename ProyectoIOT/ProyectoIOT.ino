...
// TOPICS MQTT
const char* topicGas        = "smarthome/equipo2/gas";
const char* topicSonido     = "smarthome/equipo2/sonido";
const char* topicMovimiento = "smarthome/equipo2/movimiento";
...
void loop() {
  ...
  if (now - lastMsg > interval) {
    ...
    Serial.print("\t| Gas MQ: "); Serial.print(valorGas);
    Serial.print("\t| Sonido: "); Serial.println(porcentajeSonido);

    // 5. PUBLICAR TOPICS
    client.publish(topicGas,        String(valorGas).c_str());
    client.publish(topicSonido,     String(porcentajeSonido).c_str());
    client.publish(topicMovimiento, String(distanciaCm, 1).c_str());

    // 6. JSON COMBINADO
    String payload = "{";
    payload += "\"distancia\": " + String(distanciaCm) + ", ";
    payload += "\"gas\": " + String(valorGas) + ", ";

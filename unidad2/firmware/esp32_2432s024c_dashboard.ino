/*
 * Panel de visualizacion para ESP32-2432S024C (pantalla ST7789 2.4" tipo "CYD")
 * Se conecta al mismo broker MQTT que el resto del equipo2 y muestra:
 *   - Pantalla 1: alertas y valores actuales en texto
 *   - Pantalla 2: grafico en tiempo real de gas (CO2), distancia y sonido
 * El boton BOOT (GPIO0) cambia entre ambas pantallas.
 *
 ...
 *   - ArduinoJson (v6 o v7)
 *
 * IMPORTANTE sobre TFT_eSPI: normalmente esta libreria requiere editar
 * User_Setup.h dentro de la propia libreria. Para no tener que tocar
 * archivos fuera del proyecto, el pinout se define aqui mismo ANTES de
 * incluir <TFT_eSPI.h> usando USER_SETUP_LOADED (funciona igual que
 * definirlo por build_flags en PlatformIO).
 *
 * El pinout de abajo corresponde a los modulos Sunton/Guition
 * "ESP32-2432S024C" mas comunes (SPI ST7789, sin tactil resistivo).
 * Si tu placa es una variante distinta y la pantalla queda en blanco,
 * revisa la serigrafia de la placa y ajusta estos defines.
 */
...
// Este usuario debe darse de alta en mosquitto/config/passwd y en acl.acl
// (lectura de smarthome/equipo2/alerta y smarthome/equipo2/llm/decision),
// igual que se hizo para "mkr1000", "predictor", etc.
const char* mqtt_user = "display";
const char* mqtt_pass = "esp32_display123";
const char* mqtt_client_id_prefix = "CYD-Display-";
...
// ─── Umbrales de respaldo (si no llega decision del LLM) ─
const float UMBRAL_CO2       = 400;
const float UMBRAL_SONIDO    = 80;
const float UMBRAL_DISTANCIA = 20;
...
int totalHist = 0; // cantidad de muestras validas (hasta MAX_PUNTOS)

// ─── Pantallas ─────────────────────────────────────────
enum Pantalla { PANTALLA_ALERTAS, PANTALLA_GRAFICO };
Pantalla pantallaActual = PANTALLA_ALERTAS;
...
void evaluarUmbrales() {
  ...
  if (co2 > UMBRAL_CO2 && ahora - ultimaAlertaCO2 > COOLDOWN_ALERTA_MS) {
    agregarAlerta("Gas alto: " + String(co2, 0) + " (limite " + String(UMBRAL_CO2, 0) + ")");
    ultimaAlertaCO2 = ahora;
  }
  if (sonido > UMBRAL_SONIDO && ahora - ultimaAlertaSonido > COOLDOWN_ALERTA_MS) {
    agregarAlerta("Sonido alto: " + String(sonido, 0) + "% (limite " + String(UMBRAL_SONIDO, 0) + "%)");
    ultimaAlertaSonido = ahora;
  }
  if (distancia < UMBRAL_DISTANCIA && ahora - ultimaAlertaDistancia > COOLDOWN_ALERTA_MS) {
    agregarAlerta("Distancia baja: " + String(distancia, 1) + " cm (limite " + String(UMBRAL_DISTANCIA, 0) + ")");
...
// ─── Dibujo: pantalla de alertas ───────────────────────
void dibujarPantallaAlertas(bool full) {
  if (full) {
    tft.fillScreen(TFT_BLACK);
    tft.fillRect(0, 0, tft.width(), 26, TFT_NAVY);
    tft.setTextColor(TFT_WHITE, TFT_NAVY);
    tft.setTextFont(4);
    tft.drawString("PANEL DE ALERTAS", 8, 4);
  }
  ...
  tft.setTextColor(co2 > UMBRAL_CO2 ? TFT_RED : TFT_GREEN, TFT_BLACK);
  tft.drawString("Gas (CO2): " + String(co2, 0), 8, y);

  y += 32;
  tft.fillRect(0, y, tft.width(), 30, TFT_BLACK);
  tft.setTextColor(distancia < UMBRAL_DISTANCIA ? TFT_RED : TFT_GREEN, TFT_BLACK);
  tft.drawString("Distancia: " + String(distancia, 1) + " cm", 8, y);

  y += 32;
  tft.fillRect(0, y, tft.width(), 30, TFT_BLACK);
  tft.setTextColor(sonido > UMBRAL_SONIDO ? TFT_RED : TFT_GREEN, TFT_BLACK);
  tft.drawString("Sonido: " + String(sonido, 0) + "%", 8, y);
  ...
  tft.drawString("Boton BOOT: cambiar a grafico", 8, tft.height() - 16);
}

// ─── Dibujo: pantalla de grafico ───────────────────────
void dibujarPanel(float* datos, int n, const char* etiqueta, uint16_t color, int yTop, int panelH) {
  ...
  // Valor actual a la derecha del titulo
  tft.setTextColor(color, TFT_BLACK);
...
void dibujarPantallaGrafico(bool full) {
  ...
  // Limpiar zona de graficos (dentro de los bordes de cada panel)
  ...
  tft.setTextFont(2);
  tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
  tft.drawString("Boton BOOT: cambiar a alertas", 8, tft.height() - 14);
}
...
void chequearBoton() {
  if (digitalRead(PIN_BOTON) == LOW) {
    ...
    if (ahora - ultimoCambioBoton > DEBOUNCE_MS) {
      ...
      pantallaActual = (pantallaActual == PANTALLA_ALERTAS) ? PANTALLA_GRAFICO : PANTALLA_ALERTAS;
      pantallaDirty = true;
    }
  }
}
...
void loop() {
  ...
  if (debeRedibujar) {
    if (pantallaActual == PANTALLA_ALERTAS) {
      dibujarPantallaAlertas(pantallaDirty);
    } else {
